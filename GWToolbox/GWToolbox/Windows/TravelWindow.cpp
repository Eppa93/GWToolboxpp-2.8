...
void TravelWindow::Draw(IDirect3DDevice9* pDevice) {
	...
	if (ImInPresearing()) {
		...
		if (ImGui::Begin(Name(), GetVisiblePtr(), GetWinFlags())) {
			...
			TravelButton("Rata Sum", 0, GW::Constants::MapID::Rata_Sum_outpost);
			TravelButton("Embark", 1, GW::Constants::MapID::Embark_Beach);
			TravelButton("Kaineng", 0, GW::Constants::MapID::Kaineng_Center_outpost);
			TravelButton("L.A.", 1, GW::Constants::MapID::Lions_Arch_outpost);
			TravelButton("GToB", 0, GW::Constants::MapID::Great_Temple_of_Balthazar_outpost);
			TravelButton("Vlox", 1, GW::Constants::MapID::Vloxs_Falls);
