...
#include <Widgets\SoulCounterWidget.h>
...
void ToolboxSettings::InitializeModules() {
	...
	if (use_DomainOfAnguishTimer) DomainOfAnguishTimer::Instance().Initialize();
	...
	if (use_SoulCounter) SoulCounterWidget::Instance().Initialize();
...
void ToolboxSettings::DrawSettingInternal() {
	...
	ImGui::Checkbox("SoulCount", &use_SoulCounter);
	ImGui::SameLine(ImGui::GetWindowWidth() / 2);
	ImGui::Checkbox("DoA Timer", &use_DomainOfAnguishTimer);
...
void ToolboxSettings::LoadSettings(CSimpleIni* ini) {
	...
	use_SoulCounter = ini->GetBoolValue(Name(), VAR_NAME(use_SoulCount), true);
	use_DomainOfAnguishTimer = ini->GetBoolValue(Name(), VAR_NAME(use_DomainOfAnguishTimer), true);
...
void ToolboxSettings::SaveSettings(CSimpleIni* ini) {
	...
	ini->SetBoolValue(Name(), VAR_NAME(use_SoulCount), use_SoulCounter);
	ini->SetBoolValue(Name(), VAR_NAME(use_DomainOfAnguishTimer), use_DomainOfAnguishTimer);
